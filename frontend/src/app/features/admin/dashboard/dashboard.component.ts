      ...
      <ng-container *ngIf="state.data; else dashboardError">
        <div class="flex flex-col gap-8">
          <header class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <p class="text-sm font-semibold text-slate-400 uppercase tracking-[0.3em]">
                Admin overview
              </p>
            ...
            <div class="flex items-center gap-3">
              <button
                class="rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 hover:border-primary hover:text-primary transition"
              >
                Export report
              </button>
              <button
                class="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-primary/20"
              >
                Create update
              </button>
            </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div class="mt-4 flex items-end justify-between">
                <span class="text-3xl font-bold text-slate-900">{{
                  state.data.stats.totalSolutions
                }}</span>
                <span class="text-xs font-semibold text-green-500"
                  >{{ state.data.stats.featuredSolutions }} featured</span
                >
              </div>
              <div class="mt-4 h-2 rounded-full bg-slate-100">
                <div
                  ...
                  [style.width.%]="
                    state.data.stats.totalSolutions
                      ? (state.data.stats.activeSolutions / state.data.stats.totalSolutions) * 100
                      : 0
                  "
                ></div>
              </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div class="mt-4 flex items-end justify-between">
                <span class="text-3xl font-bold text-slate-900">{{
                  state.data.stats.totalProducts
                }}</span>
                <span class="text-xs font-semibold text-amber-500"
                  >{{ state.data.stats.lowStockCount }} low stock</span
                >
              </div>
              <div class="mt-4 flex items-center gap-2 text-xs text-slate-500">
                <span class="material-symbols-outlined text-base">stacked_line_chart</span>
                {{ state.data.stats.totalCategories }} categories ·
                {{ state.data.stats.totalBrands }} brands
              </div>
            </div>
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <div class="flex items-center justify-between">
              ...
              <div class="mt-4 flex items-end justify-between">
                <span class="text-3xl font-bold text-slate-900">{{
                  state.data.stats.totalTeamMembers
                }}</span>
                <span class="text-xs font-semibold text-slate-400"
                  >{{ state.data.stats.leadershipCount }} leadership</span
                >
              </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div class="mt-4 flex items-end justify-between">
                <span class="text-3xl font-bold text-green-500"
                  >{{ state.data.stats.activeBrands }} active</span
                >
                <span class="text-xs font-semibold text-slate-400"
                  >{{ state.data.stats.activeCategories }} active categories</span
                >
              </div>
              <div class="mt-4 flex items-center gap-2 text-xs text-slate-500">
            ...
            <div class="xl:col-span-2 rounded-2xl border border-slate-200 bg-white shadow-sm">
              <div class="p-6 border-b border-slate-100">
                ...
                <p class="text-sm text-slate-500 mt-1">
                  Latest categories and brands added to the catalog.
                </p>
              </div>
              <div class="p-6 space-y-6" *ngIf="state.data.recentActivity.length; else noActivity">
                <div
                  class="flex gap-4 items-start"
                  *ngFor="let activity of state.data.recentActivity"
                >
                  <div
                    class="w-9 h-9 rounded-full flex items-center justify-center"
                    [ngClass]="activity.accent"
                  >
                  ...
                  <div>
                    ...
                    <p class="text-xs text-slate-400 mt-1">
                      {{ activity.meta }} · {{ activity.createdAt | date: 'mediumDate' }}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
              ...
              <div class="p-6 space-y-4" *ngIf="state.data.lowStock.length; else noStockAlerts">
                <div
                  ...
                  *ngFor="let product of state.data.lowStock.slice(0, 4)"
                >
                  <div>
                ...
                <div class="rounded-xl border border-slate-100 bg-slate-50 p-4">
                  ...
                  <p class="text-sm font-semibold text-slate-800 mt-2">
                    {{ state.data.stats.lowStockCount }} items below threshold
                  </p>
                </div>
              </div>
...
export class DashboardComponent {
  ...
  private buildStats(payload: {
    ...
    return {
      ...
      featuredSolutions: 0, // Property removed from Solution entity
      activeSolutions: payload.solutions.length, // All solutions are considered active
